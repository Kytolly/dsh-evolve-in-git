...
export function shouldOfferSkillPromotion(record: MemoryRecordInput): boolean {
...
export function branchNameForRecord(record: MemoryRecordInput): string {
  return `evolve/${sanitizeSegment(record.kind)}/${slugify(record.title)}`
}

export function draftSkillFromRecord(record: MemoryRecordInput): SkillDraftInput {
...
export function renderSkillDraft(draft: SkillDraftInput): SkillDraft {
  const content = [
    '---',
...
export function suggestEvolution(record: MemoryRecordInput): EvolutionSuggestion {
...
export function memoryPreview(record: MemoryRecordInput): string {
