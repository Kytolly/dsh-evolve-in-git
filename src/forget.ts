...
function archiveRootOf(config: ResolvedConfig): string {
...
function findArchived(config: ResolvedConfig, id: string): string | undefined {
  ...
  for (const file of files) {
    ...
    if (fm['id'] === id) return file
  }
  return undefined
}

/** Soft-delete an active record: move it into archiveRoot (never removed). */
export function forgetMemory(config: ResolvedConfig, id: string): ForgetResult {
...
export function restoreMemory(config: ResolvedConfig, id: string): RestoreResult {
...
function moveRecord(config: ResolvedConfig, from: string, to: string, message: string): void {
