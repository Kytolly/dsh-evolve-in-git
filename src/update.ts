...
import type { CommittedArtifact, MemoryKind, MemoryRecord, ResolvedConfig } from './types.js'
 ...
 */
function markSuperseded(config: ResolvedConfig, path: string, supersededBy: string): void {
  const raw = readFileSync(path, 'utf8')
  ...
  for (const line of match[1].split(/\r?\n/)) {
    ...
    const patchValue = key === null ? undefined : patch[key[1] as string]
    if (patchValue === undefined) {
      lines.push(line)
    } else {
      lines.push(`${key![1] as string}: ${JSON.stringify(patchValue)}`)
      seen.add(key![1] as string)
    }
  }
 ...
 * active version.
 */
export function updateMemory(config: ResolvedConfig, id: string, patch: UpdatePatch = {}): CommittedArtifact & MemoryRecord {
  ...
  const kind = (entry.kind ?? 'note') as MemoryKind
