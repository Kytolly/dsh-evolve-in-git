...
export function commitPaths(repoPath, paths, message) {
    if (paths.length === 0)
        return undefined;
...
export function writeMemoryRecord(config, input, overrides = {}) {
    const repoPath = openRepository(config);
    ...
    if (mode === 'block' && matches.length > 0) {
        throw new GitEvolutionError('sensitive content blocked by privacyMode=block: ' + matches.map((match) => match.type).join(', '), 'PRIVACY_BLOCKED');
    }
    ...
    const filePath = join(repoPath, config.memoryRoot, sanitizeSegment(input.kind), `${toIsoStamp()}-${slugify(input.title)}-${id.slice(0, 8)}.md`);
 ...
 */
export function listConflicts(repoPath) {
    const porcelain = runGit(repoPath, ['status', '--porcelain']);
    const conflicts = [];
    for (const line of porcelain.split(/\r?\n/).filter(Boolean)) {
        if (CONFLICT_CODES.test(line.slice(0, 2)))
            conflicts.push(line.slice(3).replace(/\"/g, ''));
    }
    return conflicts;
}
/**
 ...
 */
export function revertCommit(config, ref, dryRun) {
    const repoPath = openRepository(config);
    ...
    if (dryRun)
        return { dryRun: true, reverted: false, commit: undefined, wouldChange: changed };
 ...
 */
export function resolveConflict(repoPath, path, strategy) {
 ...
 */
export function branchDiff(repoPath, a, b) {
    const refB = b ?? 'HEAD';
