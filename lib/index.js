...
export class GitEvolutionService extends Service {
    ...
    constructor(ctx, config) {
        ...
        installSettingsSection(ctx, 'evolve-git', GitEvolutionService.Config, config, {
            setSource: (source) => {
                this.configSource = source;
                this.refreshConfig();
            },
            onChange: () => this.refreshConfig(),
            validate: (value) => resolveConfig(mergeConfig(this.baseConfig, value)),
        });
    ...
    refreshConfig() {
        const scopeValue = this.configSource?.() ?? {};
        this.config = resolveConfig(mergeConfig(this.baseConfig, mergeConfig(scopeValue, readConfigFile())));
    }
    registerTools(ctx) {
